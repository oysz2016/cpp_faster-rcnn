...
namespace FRCNN_API{
	...
	float get_scale(int width, int height)
	{

		if(width <= height)
		{
			return (float)600/width;
		}else
		{
			return (float)600/height;
		}
		// rertun 
	}
		...
		bool useGPU, bool ignoreLog)
	{
		...
		API::Set_Config(config_file);
                ...
                std::cout << "Set Config Success" << std::endl;
		Set_Model(proto_file, model_file);
                //caffe::Caffe::SetDevice(0);
		std::cout << "Set Model Success" << std::endl;
	}
	...
	void Detector::predict_original(const cv::Mat &img_in, std::vector<caffe::Frcnn::BBox<float> > &results) {
		...
		clock_t startTime, endTime, midTime;
		startTime = clock();
		...
		std::cout << "test_scales[0]:" << FrcnnParam::test_scales[0] << std::endl;
		std::cout << "test_max_size:" << FrcnnParam::test_max_size << std::endl;
		//float scale_factor = get_scale(img_in.cols, img_in.rows);
		midTime = clock();
                std::cout << "mid Time ----:+ " << (double)(midTime - startTime) / CLOCKS_PER_SEC << "s" << std::endl;
		cv::Mat img;
		...
		clock_t convertend = clock();
                std::cout << "convertend Time ----:+ " << (double)(convertend - midTime) / CLOCKS_PER_SEC << "s" << std::endl;		
cv::resize(img, img, cv::Size(), scale_factor, scale_factor);  //变成1000*539大小
		endTime = clock();
std::cout << "Resize Time : " << (double)(endTime - convertend) / CLOCKS_PER_SEC << "s" << std::endl;
		std::vector<float> im_info(3);
		im_info[0] = img.rows;
		im_info[1] = img.cols;
		im_info[2] = scale_factor;

		std::cout << "heighted:" << im_info[0] << std::endl;
                std::cout << "widthed:" << im_info[1] << std::endl;
                std::cout << "scaleed:" << im_info[2] << std::endl;
