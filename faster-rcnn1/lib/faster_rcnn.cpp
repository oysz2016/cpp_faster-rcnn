...
vector<vector<int>> detectionResults;
 ...
 * =====================================================================================
 */
void Detector::vis_detections(cv::Mat image, int* keep, int num_out, float* sorted_pred_cls, float CONF_THRESH, int which_class)
{
    int i=0;
...
vector<int> result;
result.pushback(which_class);
result.pushback(sorted_pred_cls[keep[i]*5+0]);
result.pushback(sorted_pred_cls[keep[i]*5+1]);
result.pushback(sorted_pred_cls[keep[i]*5+2]);
result.pushback(sorted_pred_cls[keep[i]*5+3]);
detectionResult.pushback(result);
i++;
    }
}
